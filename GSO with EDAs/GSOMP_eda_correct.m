function [ fbestvals, bestmembers, archiveNew, fvaluesNew, fvaluesAll, archiveAll,hv] = GSOMP_eda(fname, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Last modified in 2019-11-29 by Chaofan YU, Guangxi University
% 1. use the GSO algorithm and EDAs to search the Pareto front
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         end
        ...
        P=[300,0.8]; % 二维参考点
        hv(iteration,:)= hvolume2d(fvaluesNew,P); % hv指标
%% ===================== EDAs 分布估计算法 ============================================
        if iteration>6
            dev1=0;
            dev2=0;
            ...
            % iteration>6 && Bests(iteration,1)==Bests((iteration-1),1) && Bests(iteration,1)==Bests((iteration-2),1) && Bests(iteration,1)==Bests((iteration-3),1) && Bests(iteration,1)==Bests((iteration-4),1) && Bests(iteration,1)==Bests((iteration-5),1) && Bests(iteration,2)==Bests((iteration-1),2)&& Bests(iteration,2)==Bests((iteration-2),2) && Bests(iteration,2)==Bests((iteration-3),2) && Bests(iteration,2)==Bests((iteration-4),2) && Bests(iteration,2)==Bests((iteration-5),2)
               N=24;
               MaxIter1=75;
               [X1,X2,BestF1,BestF2,Iters]=eda_norm_correct(population,MaxIter1);
               bestmembers=[X1,X2];
